...
// Adapted from React Bits: https://reactbits.dev/components/stepper
...
export default function Stepper({
  ...
  const updateStep = (newStep: number) => {
    if (!beforeStepChange(newStep)) return;
      ...
      >
        ...
        {!isCompleted && (
          <div className={`rb-footer-container ${footerClassName}`}>
            <div className={`rb-footer-nav ${currentStep !== 1 ? 'rb-spread' : 'rb-end'}`}>
              {currentStep !== 1 && (
                <button
                  type="button"
                  onClick={handleBack}
                  className={`rb-back-button ${currentStep === 1 ? 'rb-inactive' : ''}`}
                  {...backButtonProps}
                >
                  {backButtonText}
                </button>
...
function StepContentWrapper({
  ...
  return (
    <motion.div
      ...
      transition={{ type: 'spring', duration: reduceMotion ? 0 : 0.4 }}
    >
    ...
    >
      <motion.div
        variants={{
          ...
          active: { scale: 1, backgroundColor: '#ff2858', color: '#ff2858' },
          complete: { scale: 1, backgroundColor: '#ff2858', color: '#ffffff' },
        }}
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    ...
    complete: { width: '100%', backgroundColor: '#ff2858' },
  };
